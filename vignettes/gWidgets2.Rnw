...
package.~\footnote{The \pkg{gWidgets} package is document in an 2007
...
\item simplifying of the code base using the powerful combination of
  ...
  easier to document, test, and deploy changes of the underlying
  packages;
...
The \pkg{devtools} package can be used to install the development
...
methods defined for them. Table~\code{tab:methods} lists most of the
...
\begin{table}
  ...
  \begin{tabular}{@{}l@{\quad}l@{}}
    ...
    \code{enabled\ASSIGN} & set if widget is sensitive to user input\\
...
(and its shortcut \code{gvbox}), \code{gframe},
...
and \code{bg} a horizontal one. (The \code{gvbox} call is a simpler to
type version of \code{ggroup} with \code{horizontal=FALSE}.) The
\code{set\_borderwidth} reference method call is not essential, but
does give some extra space around the edges of \code{g}. The
\code{svalue} method is used to set padding between children, this
call sets the border within the box. The spring is added to \code{bg}
container to right justify the buttons. When buttons are packed into
box containers, they expand to fill the direction orthogonal to the
direction of packing. As such, it is best to put buttons in a
horizontal box container, as done above.


\subsection{Widgets}
\label{sec:widgets}

Containers are needed for organizing the GUI, but more importantly are
...
arguments for a $t$-test (Figure~\ref{fig:screenshots-t-test}).  We use the convenient \code{gformlayout},
new to \pkg{gWidgets2}.
...
control how the child is laid out within the parent. For
...
interface uses \code{gcheckboxgroup} for the selection of one or more
...
Here we use the checkbox group to show the data. The items to select
...
the \code{expand=TRUE} argument here to that the widget gets the
...
button simply shows the information defined about in \code{about}. The
